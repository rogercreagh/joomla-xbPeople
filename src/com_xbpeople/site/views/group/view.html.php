...
/*******
 * @package xbPeople
 * @filesource site/views/group/view.html.php
 * @version 1.0.2.3 8th January 2023
 * @author Roger C-O
...
class XbpeopleViewGroup extends JViewLegacy {
	...
	public function display($tpl = null) {
		...
		$this->show_image = $this->params->get('show_gimage',1);
		...
		if ($this->tmpl != 'component') {
    		$srt = $app->getUserState('group.sortorder');
    		if (!empty($srt)) {
		...
		$this->item->tags = $tagsHelper->getItemTags('com_xbpeople.group' , $this->item->id);

		$document = $this->document; //Factory::getDocument();
		$document->setTitle($this->item->title);
		$document->setMetaData('title', Text::_('XBCULTURE_GROUP_CATALOGUE').' '.$this->item->title);
