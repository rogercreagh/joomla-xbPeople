...
class XbpeopleModelTag extends JModelItem {
    ...
    public function __construct($config = array()) {
        $showtags = ComponentHelper::getParams('com_xbpeople')->get('show_tags',1);
        if (!$showtags) {
...
//			$query->select('(SELECT COUNT(*) FROM #__contentitem_tag_map AS mr WHERE mr.type_alias='.$db->quote('com_xbbooks.review').' AND mr.tag_id = t.id) AS rcnt');
			...
			if ($this->item = $db->loadObject()) {
				$item = &$this->item;
				//calculate how many non xbpeople items the tag applies to to save doing it later
				$item->othercnt = $item->allcnt - ($item->pcnt + $item->chcnt);
...
// 				}
				if ($item->othercnt > 0) {
					...
					$query->where('m.type_alias NOT LIKE '.$db->quote('com_xbpeople%'));
